 ...
 *
 * Revision 4.1  20140825 geoff
 * Port to WIN32/MSVC using cmake
 *
...
static char rcs_ident[] = "$Header: compress.c,v 4.1 2014/08/25 geoff Release $";

#define ARGVAL() (*++(*argv) || (--argc && *++argv))    /* bump to next command argument, if exists */
...
static size_t total_bytes_write = 0;
...
void Usage( int lev )
{
...
#ifdef _MSC_VER
    fprintf(stderr,"--help, -h or -? = show this help and exit(0)\n");
#endif
#else 
    ...
    if (lev || debug) 
    {
        ...
        fprintf(stderr,"       If -f is not used, the user will be prompted if stdin is\n");
        fprintf(stderr,"       a tty, otherwise, the output file will not be overwritten.\n");
...
#ifndef HAVE_RINDEX_FUNC
char *rindex(char *s, char c);		/* For those who don't have it in libc.a */
#endif // !HAVE_RINDEX_FUNC
code_int getcode();
void decompress();
void output( code_int code );
void compress();

static FILE *in_stream = 0;
...
int main( int argc, char **argv )
{
...
#ifdef _MSC_VER
    /* not sure this is required, but seems safer... */
    ...
    /* allocate a file name buffer for as many argument there are... */
...
#ifndef _MSC_VER /* will not work for WIN32 since binaries have '.exe' appended, so... */
    ...
    if (*filelist != NULL) 
    {
	    for (fileptr = filelist; *fileptr; fileptr++) 
        {
	        ...
	        if (do_decomp != 0) /* DECOMPRESSION */
            {
		        ...
		        /* Open input file */
		        if ((in_stream = freopen(*fileptr, READ_MODE, stdin)) == NULL)
                {
			        perror(*fileptr); 
                    continue;
		        }
		        ...
		        if (nomagic == 0) 
                {
                    ...
                    total_bytes_read += 3;  /* read the first 3 bytes */
            ...
            {	/* COMPRESSION */
		        ...
		        /* Generate output filename */
		        strcpy(ofname, *fileptr);
#if !(defined(BSD4_2) || defined(_MSC_VER))
                /* Short filenames */
...
#endif  /* BSD4_2 | _MSC_VER - Long filenames allowed */
		        strcat(ofname, ".Z");
	        }
            ...
            {
        		if ((out_stream = freopen(ofname, WRITE_MODE, stdout)) == NULL) 
                {
		            perror(ofname);
		            continue;
		        }
            ...
            if (!quiet) {
                fprintf(stderr,"Read %lu, written %lu bytes ... ", total_bytes_read, total_bytes_write );
    	        if (do_decomp == 0)	{
                    fprintf( stderr, "compression: " );
                    prratio( stderr, total_bytes_read - total_bytes_write, total_bytes_read );
                } else {
                    fprintf( stderr, "decompression: " );
                    prratio( stderr, total_bytes_write - total_bytes_read, total_bytes_write );
                }
                fprintf(stderr,"\n");
            }
	    } /* for (fileptr = filelist; *fileptr; fileptr++) */
...
#ifdef _MSC_VER
        fprintf(stderr,"No file names found in command! Using stdin and stdout\n");
#endif
    	if (do_decomp == 0) {
	    	compress();
#ifdef DEBUG
	...
	*bp = (*bp & rmask[r_off]) | (code << r_off) & lmask[r_off];
	...
	if ( free_ent > maxcode || (clear_flg > 0))
	{
	    ...
	    if ( offset > 0 ) {
            ...
            {
			    writeerr("Write the whole buffer");
            }
    		total_bytes_write += n_bits;
	    }
	    offset = 0;
...
#endif /* DEBUG */
    	if( ferror( stdout ) ) {
	    	writeerr("output at EOF, write the rest");
        }
    }
}
...
void decompress()
{
...
#ifdef _MSC_VER
        fprintf(stderr,"Premature end of file on first getcode()\n");
#endif
    ...
    if(ferror(stdout))	{	/* Crash if can't write */
        writeerr("First decompess write");
    }
    out_count++;
    total_bytes_write++;
    ...
    while ( (code = getcode()) > -1 ) 
    {
	    ...
	    if ( (code == CLEAR) && block_compress ) 
        {
	        ...
	        if ( (code = getcode ()) == -1 ) {	/* O, untimely death! */
                fprintf(stderr, "Ugh! Untimely death!\n");
                break;
            }
	    }
	    incode = code;
...
code_int getcode() 
{
    ...
    if ( clear_flg > 0 || offset >= size || free_ent > maxcode )
    {
        ...
        total_bytes_read += size;   /* accumulate bytes read */
